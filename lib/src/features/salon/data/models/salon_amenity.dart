...
class SalonAmenity with _$SalonAmenity {
  ...
  factory SalonAmenity.fromJson(Map<String, dynamic> json) =>
      _$SalonAmenityFromJson(json);
}

extension SalonAmenityColRefExt on CollectionReference {
...
extension SalonAmenityDocRefExt on DocumentReference {
  DocumentReference<SalonAmenity> withSalonAmenityConverter() {
    return withConverter(
      fromFirestore: (snapshot, options) {
        final data = snapshot.data()!;
        data['id'] = snapshot.id;
